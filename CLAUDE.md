...
- **Vercel** deployment at `https://skillstack-learn.vercel.app` — proxies `/api/*` to homeserver via Tailscale Funnel, SPA rewrites for everything else (`vercel.json`) — auto-runs `vite build` via root `vite.config.js`
...
| **FastAPI** | Python backend on homeserver (Windows) | SQLite CRUD, Ollama AI endpoints, ChromaDB vector search, PDF/YouTube notesheet generation |
...
| `fastapi-app/pyproject.toml` | Python project manifest (uv, replaces requirements.txt) |
| `fastapi-app/uv.lock` | Deterministic lockfile (auto-generated by uv sync) |
| `fastapi-app/.venv/` | Virtual environment (managed by uv, auto-created) |
| `docs/architecture/decisions/0001-port-migration-and-uv.md` | ADR for port migration and uv adoption |
...
- **Vercel proxies /api/* to homeserver:** `vercel.json` rewrites `/api/(.*)` → `https://homeserver.buri-chromatic.ts.net/api/$1`. Browser talks same-origin to Vercel, Vercel relays to Tailscale Funnel tunnel.
- **FastAPI runs on Windows homeserver** (`192.168.1.47:8080`), exposed via Tailscale Funnel at `homeserver.buri-chromatic.ts.net`. Managed via PowerShell on the homeserver.
- **Ollama runs on homeserver** (`100.65.172.94:11434` or `localhost:11434`), accessible via Tailscale. FastAPI tries both addresses. Available models: `qwen3:8b`, `qwen2.5vl:7b`, `fin-o1:8b`, `fin-r1:7b`, `qwen3:30b-a3b`.
- **uv toolchain:** Virtual environment managed by `uv` (not pip/venv). `pyproject.toml` + `uv.lock` for deterministic builds. Recreate with `uv sync`.
- **pydantic-settings:** Dynamic `$PORT` env var injection. Port defaults to `8080`. CORS origins driven from `get_settings().cors_origins`.
- **Health probes:** `/live` (process liveness) and `/ready` (DB connectivity with 5s timeout, returns 503 if degraded).
- **PDF extraction:** 3-stage fallback — `pdftotext` (Linux), `PyMuPDF` (cross-platform, Windows), OCR via Ollama vision (scanned PDFs). 60s timeout on extraction.
