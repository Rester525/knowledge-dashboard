...
async function run() {
  ...
  const realErrors = pageErrors.filter(e => !e.includes('Offline: no local data'));
  ...
  const expectedAPIErrors = pageErrors.filter(e => e.includes('Offline: no local data')).length;
  if (expectedAPIErrors > 0) {
    info(expectedAPIErrors + ' expected offline errors (no backend deployed on Vercel)');
  }
