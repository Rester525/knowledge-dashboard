...
async function run() {
  ...
  // ── Navigate to Study → Notesheet ──
  await page.click('.nav-item[data-view="study"]');
  await wait(1000);

  // ── Create a test PDF on disk ──
  ...
  // Check drop label changed
  const dropLabel = await page.textContent("#ns-drop-label");
  if (dropLabel && dropLabel.includes("PDF selected")) ok("PDF file accepted by dropzone UI");
  else fail("Dropzone did not update after file select. Label: " + dropLabel);
  ...
  const progressDisplay = await page.getAttribute("#ns-progress", "style");
  if (progressDisplay && !progressDisplay.includes("display:none")) ok("Progress bar visible");
  else fail("Progress bar not visible after generate click");

  // ── Wait for result ──
  console.log("    Waiting for PDF notesheet result (Ollama may take 30-60s)...");
  ...
  for (let i = 0; i < 120; i++) {
    ...
    if (statusText && (statusText.includes("Failed") || statusText.includes("Error"))) {
      ...
      console.log("    Result area:", resultText?.substring(0, 500));
      break;
    }
  }
  ...
  const relevantErrors = pageErrors.filter(e =>
    !e.includes("no local data") && !e.includes("Offline")
  );
