...
async function run() {
  ...
  const exportBtn = await page
    .locator('button:has-text("Export All Data")')
    .count();
  ...
  const hasRetro = await page.evaluate(() =>
    document.documentElement.classList.contains("theme-retro"),
  );
  ...
  await bitCard.click();
  await wait(500);
  const has8Bit = await page.evaluate(() =>
    document.documentElement.classList.contains("theme-8bit"),
  );
  ...
  await wait(300);
  const accentGreen = await page.evaluate(() =>
    document.documentElement.classList.contains("accent-green"),
  );
  ...
  await wait(1000);
  const overlayGone = await cleanPage.evaluate(
  ...
  await wait(500);
  const authFormVisible = await cleanPage
    .locator("#login-auth-form")
    .isVisible();
  ...
  await wait(300);
  const isSignUp = await cleanPage.evaluate(
    () => document.getElementById("login-auth-mode").dataset.mode === "signup",
  );
